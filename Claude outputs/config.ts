...
export interface Config {
  ...
  // apiUrl: when set (or auto-derived for a ZeptoMail host), email is sent over the HTTPS API on port 443
  // instead of SMTP — required on hosts (e.g. Render free tier) that block outbound SMTP ports.
  email: { host: string; port: number; user: string; pass: string; from: string; apiUrl: string };

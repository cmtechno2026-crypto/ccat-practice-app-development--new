...
2. CLIENT HINT (non-blocking, warn only — does NOT disable submit): NEW `apps/web/src/lib/pin.ts` (mirror of the server list — the two packages share no common TS module, so intentionally duplicated; server is the authority). `RegisterScreen.tsx` (checks full DOB incl. day) + `RecoveryScreen.tsx` (no DOB on that screen) show `WEAK_PIN_HINT` on the PIN field.
3. LOCKOUT DoS + GUARDIAN ALERT: `auth.ts` — on the FIRST trip of the existing 5-attempt / 5-min lock (pre-increment failed_attempts == 4), fire ONE fire-and-forget guardian email ("sign-in temporarily locked, unlocks automatically") via the existing `sendEmail`. Throttle = the trip condition itself: failed_attempts resets to 0 only on a successful login, so re-locks after the window (pre-increment >= 5) do NOT re-alert → at most one email per attack burst, **NO throttle column / NO migration**. Skips preview accounts; silent no-op when email unconfigured; never awaited, never fails login. Flat 5-min lock kept. TODO left in code: CAPTCHA/PoW after N lockouts (future, not built).
NO SCHEMA CHANGE. NO migration. Files: gateway lib/pin.ts (new), errors.ts, routes/registration.ts, routes/recovery.ts, routes/auth.ts; web lib/pin.ts (new), screens/RegisterScreen.tsx, screens/RecoveryScreen.tsx.
BUILD CHECK PENDING (operator): Claude could NOT run it — device shell offline, cloud lacks the monorepo/node_modules. Run `pnpm install` (nodemailer) then `npx --yes pnpm@10 --filter @ccat/gateway build` and `--filter @ccat/web build`. Manual review: types expected clean (pin.ts is dependency-free; `WEAK_PIN` code returned via existing AppError; imports use `.js` specifiers in gateway per ESM; Field hint/hintKind already used elsewhere in both web screens).

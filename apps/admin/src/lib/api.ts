...
export const api = {
  ...
  teacherBookingLinkPreview: (teacher_ids: string[], grade: number, subject: string) => req<{ available: number; teachers: number }>('GET', `/v1/admin/teacher/booking-links/preview?teacher_ids=${encodeURIComponent(teacher_ids.join(','))}&grade=${grade}&subject=${encodeURIComponent(subject)}`),
  ...
  teacherCreateBookingLink: (b: { teacher_ids: string[]; grade: number; subject: string; label?: string; expires_in_days?: number | null; never_expires?: boolean }) => req<any>('POST', '/v1/admin/teacher/booking-links', b),

...
export const api = {
  ...
  teacherBookingLinkPreview: (teacher_ids: string[], combos: { subject: string; grade: number }[]) => req<{ available: number; teachers: number; combos: number }>('GET', `/v1/admin/teacher/booking-links/preview?teacher_ids=${encodeURIComponent(teacher_ids.join(','))}&combos=${encodeURIComponent(JSON.stringify(combos))}`),
  ...
  teacherBookingRequests: (status?: string, link_id?: string) => { const p = new URLSearchParams(); if (status) p.set('status', status); if (link_id) p.set('link_id', link_id); const qs = p.toString(); return req<{ requests: any[] }>('GET', `/v1/admin/teacher/booking-requests${qs ? '?' + qs : ''}`); },

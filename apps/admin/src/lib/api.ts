...
export const api = {
  ...
  // Parent Booking Links (A) + Booking Requests inbox (B). Teacher Hub site; gated server-side by teacher.* perms.
  teacherBookingLinkPreview: (teacher_ids: string[], combos?: { subject: string; grade: number }[]) => req<{ available: number; teachers: number; combos: number }>('GET', `/v1/admin/teacher/booking-links/preview?teacher_ids=${encodeURIComponent(teacher_ids.join(','))}${combos && combos.length ? '&combos=' + encodeURIComponent(JSON.stringify(combos)) : ''}`),
  teacherBookingLinks: (status?: string) => req<{ links: any[] }>('GET', `/v1/admin/teacher/booking-links${status ? '?status=' + encodeURIComponent(status) : ''}`),
  teacherCreateBookingLink: (b: { teacher_ids: string[]; combos?: { subject: string; grade: number }[]; label?: string; expires_in_days?: number | null; never_expires?: boolean }) => req<any>('POST', '/v1/admin/teacher/booking-links', b),
  teacherPatchBookingLink: (id: string, b: { action?: 'revoke' | 'activate'; expires_in_days?: number | null; never_expires?: boolean }) => req<any>('PATCH', `/v1/admin/teacher/booking-links/${id}`, b),
  teacherBookingRequestsPending: () => req<{ pending: number }>('GET', '/v1/admin/teacher/booking-requests/pending-count'),
  ...
  trainingDeleteRoleplay: (id: number) => req<{ deleted: number }>('DELETE', `/v1/admin/training/roleplays/${id}`),
  trainingReorderRoleplays: (ids: number[]) => req<{ roleplays: any[] }>('POST', '/v1/admin/training/roleplays/reorder', { ids }),
  teacherSetBan: (id: string, banned: boolean) => req<{ id: string; banned_at: string | null }>('POST', `/v1/admin/teacher/teachers/${id}/${banned ? 'ban' : 'unban'}`, {}),
  teacherDeleteTeacher: (id: string) => req<{ deleted: string }>('DELETE', `/v1/admin/teacher/teachers/${id}`),
  ...
  getEntitlement: (email: string) => req<{ item: any; students: any[]; allowed_tiers: string[]; grant_reasons: string[] }>('GET', `/v1/admin/entitlements?email=${encodeURIComponent(email)}`),
  // Paid entitlements that have no account yet (landing Case 2: paid before signing up). They drop off
  // this list once the buyer creates an account with that email.
  unclaimedPaid: () => req<{ items: any[] }>('GET', '/v1/admin/entitlements/unclaimed'),
  setEntitlement: (b: { guardian_email: string; tier: 'free' | 't50' | 't250' | 't500'; status?: string; current_period_end?: string | null; grant_reason?: string }) => req<{ item: any }>('POST', '/v1/admin/entitlements', b),
  // Site-wide default plan (promo lever) — Payments admin control.
  getDefaultPlan: () => req<{ default_tier: string; default_until: string | null }>('GET', '/v1/admin/settings/default-plan'),
  setDefaultPlan: (b: { tier: 'free' | 't50' | 't250' | 't500'; until?: string | null }) => req<{ default_tier: string; default_until: string | null }>('PUT', '/v1/admin/settings/default-plan', b),
  // Per-student membership (guardian resolved server-side from the student id).
  getStudentMembership: (id: string) => req<{ guardian_email: string | null; effective: { tier: string; source: string; current_period_end: string | null; promo_active: boolean; default_tier: string }; item: any; allowed_tiers: string[]; grant_reasons: string[] }>('GET', `/v1/admin/students/${id}/membership`),
  setStudentMembership: (id: string, b: { tier: 'free' | 't50' | 't250' | 't500'; grant_reason?: string; until?: string | null }) => req<{ item: any; guardian_email: string }>('POST', `/v1/admin/students/${id}/membership`, b),
  // rewards
  ...
  editAchievement: (versionId: string, b: { name?: string; xp?: number | null; coins?: number | null; active?: boolean; repeatable?: boolean }) => req<any>('PATCH', `/v1/admin/rewards/achievements/versions/${versionId}`, b),
